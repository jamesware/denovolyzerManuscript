...
-  `dnm.genes`: a vector of names of genes that contain *de novo* variants
...
The same three arguments are required as for BASIC PROTOCOL 1: `dnm.genes` (vector of names of genes containing *de novo* variants), `dnm.classes` (a vector of variant consequences) and `nsamples` (number of samples).  In addition, `nperms` determinines the number of permutations run (defaults to 100).
...
2) Call the `denovolyzeByGene` function.The same three arguments are required as for the previous protocols: `dnm.genes` (vector of names of genes containing *de novo* variants), `dnm.classes` (a vector of variant consequences) and `nsamples` (number of samples).
