...
The variant classes must be labelled using the following terms:  
...
**TO INSERT - Gene Names usage & identifiers**
...
Given that the number of de novo variants per trio follows a Poisson distribution{PMID:22495311}, we use the Poisson distribution to evaluate the excesses of de novo events. As an example, to determine if a particular gene has more de novo loss-of-function variants than expected, we multiply that gene’s probability of a loss-of-function mutation by the number of trios and by 2 (for the number of chromosomes) in the study. The resulting number is used as the expectation (lambda value) to which the observed number of de novo loss-of-function variants. Specifically, the `ppois` command in R is used. The three de novo loss-of-function variants seen in DYRK1A are used in the example below. Note that, given the way that `ppois` works, one is subtracted from the observed number when expectation is lower than observation.
```{r}
n_lof_dyrk1a <- 3
prob_lof_dyrk1a <- 2.52829677579e-06
n_trios <- 1078
ppois(n_lof_dyrk1a-1, prob_lof_dyrk1a*n_trios*2, lower=FALSE)
```

denovolyze yields the same result:
```{r}
denovolyzeByGene(genes=autismDeNovos$gene,
                 classes=autismDeNovos$class,
                 nsamples=1078,
