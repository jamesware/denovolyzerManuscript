...
**denovolyzeR** will output one line for every gene that contains at least one variant in  the input data.  Rather than using `head`, users might note that the `dplyr` package, which is required in order to install **denovolyzeR**, provides a convenient `filter` function that allows us to only display samples with multiple protein-altering variants, for example
```{r, include=FALSE}
library(dplyr)
```
